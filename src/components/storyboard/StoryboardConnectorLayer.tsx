"use client";
import { SceneCard, Connector } from "@/lib/storyboard-api";
...
function getCenter(el: SceneCard): { x: number; y: number } {
  return {
    x: (el.x || 0) + (el.width || 320) / 2,
    y: (el.y || 0) + (el.height || 180) / 2,
  };
}

export function StoryboardConnectorLayer({
  cards, connectors, onRemoveConnector,
}: Props) {
  ...
  return (
    ...
    >
      ...
      {connectors.map((conn) => {
        const fromEl = cards.find((e) => e.id === conn.fromId);
        const toEl = cards.find((e) => e.id === conn.toId);
