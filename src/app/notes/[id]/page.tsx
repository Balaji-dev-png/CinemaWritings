...
import {
  Home, ArrowLeft, Layers, LayoutGrid, Plus, Trash2,
  Pin, PinOff, BookOpen, Search, X, Check, Palette,
} from "lucide-react";
import {
  ApiNote,
  apiGetNotes,
...
export default function NotesPage() {
  ...
  // Load script title and notes
  useEffect(() => {
    getScriptById(scriptId).then((s) => {
      if (s) setScriptTitle(s.title || "Script");
    });
    apiGetNotes(scriptId)
      .then((data) => {
        setNotes(data);
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0d0d0d] text-white overflow-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <header className="flex items-center gap-3 px-5 py-3 shrink-0 border-b border-white/8 bg-[#111]/80 backdrop-blur-xl z-30">
        ...
        <button onClick={() => navigateTo(`/editor/${scriptId}`, "Back to Editor...")} className="text-xs text-zinc-500 hover:text-white transition-colors flex items-center gap-1">
          <ArrowLeft className="w-3.5 h-3.5" /> Editor
        </button>
        <span className="text-zinc-800">|</span>
        <button onClick={() => navigateTo(`/directors-suite/${scriptId}`, "Opening Director's Suite...")} className="text-xs text-zinc-500 hover:text-white transition-colors flex items-center gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold tracking-wide" style={{ color: "#c9a84c" }}>Notes</span>
          <span className="text-zinc-600 text-xs">— {scriptTitle}</span>
        </div>
            ...
            {loading ? (
              ...
              filtered.map((note) => (
                <motion.div
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-xl transition-all group relative cursor-pointer ${
                    activeId === note.id
                  ...
                  <div className="flex items-start justify-between gap-1">
                    ...
                    <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                      <button
                        ...
                        className="text-zinc-600 hover:text-[#c9a84c] transition-colors"
            ...
            {activeNote ? (
              ...
              >
                ...
                <div className="flex items-center gap-3 px-8 pt-8 pb-4 shrink-0">
                  ...
                  >
                    {activeNote.pinned
                      ? <PinOff className="w-4 h-4" style={{ color: "#c9a84c" }} />
              ...
              >
                ...
                <div>
                  <p className="text-white font-semibold">No note selected</p>
                  <p className="text-zinc-500 text-sm mt-1">Create a note or select one from the sidebar</p>
                </div>
                <button
                  onClick={handleCreate}
